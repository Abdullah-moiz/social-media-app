...
export default function ProfilePage() {
  ...
  const [profileData, setProfileData] = useState({ name: user?.name || "", dob: user?.dob || "", phoneNumber: user?.phoneNumber || "", bio: user?.bio || "", background: null, profile: null })
    ...
    <div className='w-full py-4 bg-gray flex flex-col items-center justify-center bg-base-200 '>
      <form onSubmit={handleUpdateProfile} className='w-3/4 flex flex-col items-center justify-center'>
        ...
        <div className="form-control w-full max-w-xs">
          ...
          <textarea value={profileData?.bio || "Add Bio"} onChange={(e) => setProfileData({ ...profileData, bio: e.target.value })} disabled={edit} className="textarea" placeholder="Bio"></textarea>
        </div>
        <div className="form-control w-full max-w-xs">
          <label className="label">
        ...
        <div className="form-control w-full max-w-xs">
          ...
          <input value={profileData?.dob || "Add DOB"} onChange={(e) => setProfileData({ ...profileData, dob: e.target.value })} disabled={edit} type="text" placeholder="Type here" className="input input-bordered w-full max-w-xs" />
        </div>
        <div className="form-control w-full max-w-xs">
          <label className="label">
            <span className="label-text">Phone Number</span>
          </label>
          <input value={profileData?.phoneNumber || "Add Phone Number "} onChange={(e) => setProfileData({ ...profileData, phoneNumber: e.target.value })} disabled={edit} type="text" placeholder="Type here" className="input input-bordered w-full max-w-xs" />
        </div>
        <div className="form-control w-full max-w-xs">
          <label className="label">
            <span className="label-text">Change Profile</span>
          </label>
          <input accept='image/*' onChange={handleProfileImage} disabled={edit} type="file" placeholder="Type here" className="file-input file-input-bordered w-full max-w-xs" />
        </div>
        <div className="form-control w-full max-w-xs">
          <label className="label">
            <span className="label-text">Change Background</span>
          </label>
          <input  accept='image/*' onChange={handleBackgroundImage} disabled={edit} type="file" placeholder="Type here" className="file-input file-input-bordered w-full max-w-xs" />
        </div>
