...
export const AddNewComment = async (formData) => {
    try {
        ...
        console.log('error in adding new comment  (service) => ', error);
    }
}
...
export const getCommentsOFPost = async (id) => {
    try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/comment?id=${id}`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${Cookies.get('token')}`,
            },
        ...
        console.log('error in getting specified Post  comments (service) => ', error);
    }
}